...
const Dashboard = () => {
  ...
  const originalData = [
    { n: "Adi Saputra", nim: "2011080518", p: "Teknik Informatika", t: "2024", s: "Valid", c: "bg-[#F39191]", ini: "AS" },
    { n: "Rani Maharani", nim: "2211080518", p: "Akuntansi", t: "2026", s: "Proses", c: "bg-[#8BC9F3]", ini: "RM" },
    { n: "Budi Pratama", nim: "1811080518", p: "Manajemen Bisnis", t: "2022", s: "Valid", c: "bg-[#E691F3]", ini: "BP" },
    { n: "Kayla Kay", nim: "2011080518", p: "Ilmu Hukum", t: "2024", s: "Reject", c: "bg-[#91F3A0]", ini: "KK" },
  ];
  ...
  useEffect(() => {
    // Set filteredData to all data when component first loads or when ijazahList changes
    if (ijazahList.length > 0) {
      setFilteredData(ijazahList);
    } else if (searchQuery === "") {
      setFilteredData(originalData);
    }
  }, [ijazahList, searchQuery]);
  ...
  const fetchAllData = async () => {
    ...
    try {
      ...
      // Store API data but also set filteredData
  ...
  // Separate useEffect for filtering to prevent UI blinking
  useEffect(() => {
    // Use API data if available, otherwise use originalData
    ...
    if (searchQuery.trim() === "") {
      ...
      const filtered = dataToFilter.filter(item => {
        const name = (item.n || item.nama || "").toLowerCase();
  ...
  return (
    <DashboardLayout>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-10">
        <StatCard
          title="Total Ijazah Terbit"
          value={stats?.totalIjazahTerbit}
          sub={`+${stats?.perubahanBulanTerakhir || 0}% dari bulan lalu`}
          subColor="text-green-500"
          icon="🎓"
          trend={stats?.perubahanBulanTerakhir}
        />
        <StatCard
          title="Permintaan Verifikasi"
          value={stats?.permintaanVerifikasi}
          sub={`${stats?.permintaanBaruHariIni || 0} permintaan baru hari ini`}
          subColor="text-yellow-500"
          icon="📝"
        />
        <StatCard
          title="Data Reject"
          value={stats?.dataReject}
          sub={`${stats?.rejectMingguIni || 0} Data ditolak Minggu ini`}
          subColor="text-red-500"
          icon="❌"
        />
        <StatCard
          title="Data Revoke"
          value={stats?.dataRevoke}
          sub={`Tidak ada perubahan hari ini`}
