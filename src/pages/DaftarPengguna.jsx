...
import { FiPlus, FiEdit2, FiTrash2, FiChevronDown, FiEye, FiEyeOff, FiCheckCircle } from "react-icons/fi";
        ...
        <div className="fixed inset-0 bg-black/40 flex justify-center items-center z-50">
          <div className="bg-white w-[400px] rounded-2xl shadow-xl overflow-hidden">
            <div className="p-8 flex flex-col items-center text-center gap-4">
              ...
              <h2 className="text-xl font-bold text-gray-800">Data Berhasil Disimpan</h2>
              <p className="text-sm text-gray-500">Data pengguna telah berhasil disimpan ke dalam sistem.</p>
            </div>
...
const AddUserForm = ({ onSave, onClose, users = [] }) => {
  ...
  const isSubmitDisabled =
    !role || !namaUnit || !form.nama || !form.nidn || !form.email ||
    !form.password || form.password.length < 8 || !personilData?.nama;
...
const EditUserForm = ({ userData, onSave, onClose }) => {
  ...
  const passwordKurang = form.password.length > 0 && form.password.length < 8;
  const isSubmitDisabled = !form.email || !form.password || form.password.length < 8;
